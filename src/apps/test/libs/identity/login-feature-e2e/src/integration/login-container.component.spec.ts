import {
  ...
  getServerError,
  getUserName,
} from '@zapisywarka-client-aps/web-identity-utills';

describe('identity-login-feature', () => {
  beforeEach(() =>
    cy.visit('/iframe.html?id=logincontainercomponent--default')
  );
  ...
  it('should show server error', () => {
    ...
    cy.intercept('POST', '/users/login', { forceNetworkError: true }).as(
      'loginReq'
    );
