...
import { CatalogQuery, CatalogItem, CatalogItemView, CatalogService } from '@zapisywarka-client-aps/catalog/domain-catalog-item';
...
export class CatalogListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.catalogQuery.selectHasCache().pipe(switchMap(hasCache => {
  ...
  remove(row: CatalogItem) {
    this.dialog.open(DeleteConfirmationComponent, {context: {message: `Czy potwierdzasz usunięcie pozycji: ${row.name}?`}})
