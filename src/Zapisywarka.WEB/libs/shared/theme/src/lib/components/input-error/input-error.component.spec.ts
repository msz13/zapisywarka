...
import { SpectatorHost, createHostFactory, createComponentFactory, Spectator } from '@ngneat/spectator/jest'
...
describe('input error component', () => {
  ...
  it('should display multiple errors', () => {
   const errors = {
    ...
    expect(spectator.queryAll('.control-error')[1]).toContainText("Should be valid email")
    
  });

  
  it('should throw when there is no error', () => {
    expect(()=> {createComponent()}).toThrowError('Form control is empty')
  ...
  it('should display defould message when when there is no message', () => {
    ...
    expect(spectator.queryAll('.control-error').length).toBe(1)
    expect(spectator.query('.control-error')).toContainText('Nieproprawne dane')
  });
