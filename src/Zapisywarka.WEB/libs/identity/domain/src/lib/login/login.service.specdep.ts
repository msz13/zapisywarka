...
const setFormValues = (
  form: IFormGroup<LoginCredentials>,
...
describe('LoginService', () => {
  ...
  describe('request validation', () => {
    it('when username is empty should return form error', () => {
      ...
      expect(form.controls.userName.errorMessage).toMatch(
    ...
    it('when password is empty should return form error', () => {
      ...
      expect(form.controls.password.errorMessage).toMatch(
        'Hasło jest wymagane'
      );
    });
    ...
    it('when login or password is invalid should return form error', () => {
      ...
      req.flush(
        { message: errMsg },
        { status: 401, statusText: 'Unauthorised request' }
      );
