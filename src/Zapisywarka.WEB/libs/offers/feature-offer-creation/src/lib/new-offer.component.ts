...
export class NewOfferComponent implements OnInit {
  ...
  get offerBasicData() {
    ...
    return this.formsManager.getControl('offerBasicData') as unknown as FormGroup
 ...
 constructor(private dateService: NbDateService<Date>, 
  ...
  ngOnInit(): void {
    this.formsManager.valueChanges('offerBasicData').subscribe(form => this.offer = form)
    this.$offerItems = this.formsManager.controlChanges('offerItems').pipe(map(f => f.rawValue ))
  }
 
 submit(): void {
    console.log('from button'+JSON.stringify(this.formsManager.getControl('offerItems').value))
  } 
