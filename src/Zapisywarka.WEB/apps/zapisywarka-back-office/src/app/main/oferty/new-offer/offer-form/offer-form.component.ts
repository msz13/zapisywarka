...
export class OfferFormComponent implements OnInit {
    ...
    this.offerForm = this.fb.group({
      offerName: [
        ...
        [
          ...
          RxwebValidators.minLength({
            value: 3,
            message: 'Nazwa oferty musi mieć minimalnie {{1}} znaki',
          }),
          RxwebValidators.maxLength({
            value: 60,
            message: 'Nazwa oferty musi mieć maksymalnie 60 znaków',
          }),
        ],
      ],
      endOfferDate: [
        ...
        [
          RxwebValidators.required({
            message: OFFER_MSG.END_OFFER_DATE_REQUIRED,
          }),
        ],
      ],
      collectionDate: this.fb.group({
        from: [
          new Date(),
          RxwebValidators.required({
            message: OFFER_MSG.COLLECTION_FROM_DATE_REQUIRED,
          }),
        ],
        to: [new Date()],
      }),
  ...
  get offerName() {
    return this.offerForm.get('offerName');
  }
  get endOfferDate() {
    return this.offerForm.get('endOfferDate');
  }
  get form() {
    return this.offerForm.get('collectionDate').get('from');
  }
  get to() {
    return this.offerForm.get('collectionDate').get('to');
  }
