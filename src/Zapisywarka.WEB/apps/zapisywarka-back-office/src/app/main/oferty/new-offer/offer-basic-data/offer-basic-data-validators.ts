...
export const endCollectionGraterThanStart: ValidatorFn = (
  ...
  return start && end && end.getTime() < start.getTime()
    ? {
        endCollectionGraterThanStart: {
          message:
            'Data zakończenia odbioru nie może być mniejsza niż rozpoczęcia',
        },
      }
    : null;
};

export const endCollectionEarlierThanEndOffer: ValidatorFn = (
  control: AbstractControl
): ValidationErrors | null => {
  ...
  return endOffer &&
    endCollection &&
    endCollection.getTime() < endOffer.getTime()
    ? {
        endCollectionGraterThanStart: {
          message:
            'Data zakończenia odbioru nie może być wcześniejsza niż zakończenia zapisów',
        },
      }
    : null;
};
...
export class UniqueOfferNameValidator {
  ...
  validate(name: string): Observable<ValidationErrors | null> {
    return this.offersService.isNameValid(name).pipe(
      map((isValid) =>
        !isValid
          ? {
              uniqueOfferName: {
                message: `Nazwa oferty '${name}' została już użyta`,
              },
            }
          : null
      ),
