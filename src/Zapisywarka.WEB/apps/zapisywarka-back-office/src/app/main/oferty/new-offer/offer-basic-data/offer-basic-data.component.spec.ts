import { Spectator, createComponentFactory, byTextContent } from '@ngneat/spectator/jest';
...
describe('nameComponent', () => {
  ...
  const createComponent = createComponentFactory({
    ...
    imports: [ThemeModule.forRoot(), ReactiveFormsModule, RxReactiveFormsModule, NbDatepickerModule.forRoot()]
  });
  ...
  it.only('fill form group', ()=>{
      ...
      spectator.typeInElement(offerBasicData.name, '[data-cy="offer-name"] input')
      spectator.typeInElement(offerBasicData.endOfferDate.toISOString(), '[data-cy="end-offer-date"] input')
      spectator.typeInElement(offerBasicData.startCollectionDate.toISOString(), '[data-cy="start-collection"] input')
      spectator.typeInElement(offerBasicData.endCollectionDate.toISOString(), '[data-cy="end-collection"] input')

      const formValue = spectator.component.offerForm.value
      const formsManagerValue = spectator.component.formsManager.getControl('offerBasicData').value
  ...
  it('set values to form manager', fakeAsync(()=>{
    ...
    let actual
    formsManager.valueChanges('offerBasicData').subscribe(form => actual=form)

    sut.offerForm.get('name').setValue(offerBasicData.name)
    sut.offerForm.get('endOfferDate').setValue(offerBasicData.endOfferDate)
    sut.offerForm.get('startCollectionDate').setValue(offerBasicData.startCollectionDate)
    sut.offerForm.get('endCollectionDate').setValue(offerBasicData.endCollectionDate)
  ...
  it("show name is required error", ()=>{
    ...
    expect(spectator.query('.control-error')).toHaveText("Wpisz nazwę oferty - pole jest wymagane")
