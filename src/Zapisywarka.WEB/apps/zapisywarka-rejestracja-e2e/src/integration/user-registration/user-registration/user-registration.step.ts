...
And(
  ...
  (registrationData) => {
    const {
      ...
      Hasło: password,
      Potwierdzenie_hasła: passwordConfirmation,
    } = registrationData.hashes()[0];

    driver.enterRegistrationData(
      uniqueName(userName),
      password,
...
Then(
  'Baza użytkowników zawiera organizatora zapisów o imieniu {string}',
  (userName) => {
    restDriver
      .getOrganisers()
      .should('deep.include', { username: uniqueName(userName) });
  }
);

Then(
  'Nie może procedować rejestracji i widzi komunikat {string}',
  (userName) => {
